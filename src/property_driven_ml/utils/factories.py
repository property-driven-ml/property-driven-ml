...
from examples.alsomitra_dataset import AlsomitraInputRegion
from .safe_eval import safe_bounds


def CreateEpsilonBall(eps: float) -> Tuple[Callable[..., constraints.EpsilonBall], Callable[..., constraints.EpsilonBall]]:
    """Create epsilon ball constraints for train and test."""
    ...
    def create_eps_ball(dataset, mean, std):
...
def CreateAlsomitraInputRegion(
    ...
    x: Optional[str] = None, 
    y: Optional[str] = None
) -> Tuple[Callable[..., AlsomitraInputRegion], Callable[..., AlsomitraInputRegion]]:
    ...
    def bounds_fn(input_tensor: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        bounds = []
        for param_name, param_expr in [('v_x', v_x), ('v_y', v_y), ('omega', omega), ('theta', theta), ('x', x), ('y', y)]:
        ...
        lo_tensor = torch.tensor(bounds[::2], device=input_tensor.device, dtype=input_tensor.dtype)
        hi_tensor = torch.tensor(bounds[1::2], device=input_tensor.device, dtype=input_tensor.dtype)
