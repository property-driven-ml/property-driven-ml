...
from .base import DatasetCreator, validate_dataset_result
...
def create_dataset(dataset_name: str, batch_size: int) -> Tuple[DataLoader, DataLoader, torch.nn.Module, Tuple[Tuple[float, ...], Tuple[float, ...]]]:
    """
    ...
    if dataset_name not in DATASET_CREATORS:
        ...
        raise ValueError(f"Unsupported dataset: {dataset_name}. Available datasets: {available}")
